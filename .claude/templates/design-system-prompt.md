# [APP NAME] - Complete Design System Reference

You are a design-system-aware assistant. This document is the single source of truth for every visual element, token, pattern, and convention used in this application's UI. Always reference this when building, reviewing, or modifying components.
...
### 2.2 Extended Accent Palette (tailwind.config.ts)
...
|---------|-----------|--------|------|
| Success | `bg-green-50` | `border-green-200` | `text-green-600` / `text-green-700` |
| Warning | `bg-yellow-50` | `border-yellow-200` | `text-yellow-600` / `text-yellow-700` |
| Error | `bg-red-50` | `border-red-200` | `text-red-700` / `text-destructive` |
...
| `shadow-xl` | Feature form cards, dropdown menus |
...
**Gradient CTA** (used in public/marketing pages):
```
...
| **Content** | `fixed left-[50%] top-[50%] z-50 w-full max-w-lg translate-x-[-50%] translate-y-[-50%] gap-4 border bg-background p-6 shadow-lg rounded-lg` |
...
### 8.22 Form (`/components/ui/form.tsx`)

| Part | Classes |
|------|---------|
| **FormItem** | `space-y-2` |
| **FormLabel** | inherits Label + `text-destructive` on error |
...
### 9.1 Authenticated Dashboard Shell

```
+--------------------------------------------------+
|  h-16 sticky top-0 z-40 border-b bg-background/95 backdrop-blur shadow-sm
|  [ Mobile menu ] [ Search (w-64) ] [ Notifications ] [ User menu ]
+----------+---------------------------------------+
| Sidebar  |                                       |
...
- Footer: `border-t bg-neutral-900 text-white py-12`
- Social icons: `h-10 w-10 rounded-full bg-neutral-800 hover:bg-cyan-600`
- Footer links: `text-sm text-neutral-400 hover:text-cyan-400`
...
| Content | `FileText`, `BookOpen`, `Layers`, `Target` |
...
- Focus states always visible (`ring-2 ring-cyan-500` for custom focus)
...
| Login/Auth | `bg-gradient-to-br from-cyan-50 via-white to-blue-50` |
| CTA section | `bg-gradient-to-br from-cyan-500 via-blue-500 to-magenta-500` |
| Table header | `bg-gradient-to-r from-cyan-500 to-blue-500` |
...
## 16. PUBLIC/MARKETING PAGE PATTERNS
...
### Email Capture CTA
```
bg-gradient-to-br from-cyan-500 via-blue-500 to-magenta-500
rounded-2xl p-12 text-center shadow-2xl
Input: bg-white/10 backdrop-blur-sm text-white border-white/20 placeholder:text-white/60
Button: bg-white text-cyan-600 hover:bg-neutral-100
```
...
## 19. SETUP INSTRUCTIONS FOR NEW PROJECTS
...
```bash
npm install framer-motion lucide-react react-hook-form @hookform/resolvers zod sonner clsx tailwind-merge
...
```bash
npx shadcn@latest add button card badge input textarea label select table tabs dialog sheet dropdown-menu tooltip popover progress switch checkbox alert separator scroll-area form
```

### Step 5: Copy Configuration Files
1. Copy the `tailwind.config.ts` extended colors (cyan, blue, magenta, yellow, neutral) and keyframes
2. Copy the `globals.css` CSS variables (light + dark theme)
3. Copy the `cn()` utility to `src/lib/utils.ts`
4. Set up Inter font in `layout.tsx`

### Step 6: Copy These Agent Files
```bash
mkdir -p .claude/agents
cp design-system.yml .claude/agents/
cp design-system-prompt.md .claude/agents/
# Update [APP NAME] in the prompt file header
```

### Step 7: Build Your Pages
Use the patterns in this document for:
- Public layout (Section 9.2) + marketing pages (Section 16)
- Dashboard shell (Section 9.1) + authenticated pages
- All components follow the catalog (Section 8)
