...
export default function SalesPackagePage() {
  // Source selection
  const [sourceMode, setSourceMode] = useState<'portfolio' | 'excel'>('portfolio');
  ...
  const [excelUploadStatus, setExcelUploadStatus] = useState<'idle' | 'uploading' | 'processing' | 'ready' | 'error'>('idle');
  const [excelUploadError, setExcelUploadError] = useState<string>('');
  const [excelPatentCount, setExcelPatentCount] = useState<number>(0);
  ...
  useEffect(() => {
    apiClient.get<{ results?: Portfolio[] } | Portfolio[]>('/patents/portfolios/?limit=100')
      .then(res => {
        ...
        setPortfolios(list);
        if (list.length === 1) setSelectedPortfolioId(list[0].id);
      })
      .catch(() => {});
  }, []);

  // ── Resolve project from portfolio ────────────────────────────────────────
  useEffect(() => {
    if (sourceMode !== 'portfolio') return;
    if (!selectedPortfolioId) { setProjectId(''); setResult(null); setAttributes([]); return; }
    analyticsApi.getOrCreateProjectFromPortfolio(selectedPortfolioId)
      .then(res => {
      ...
      .catch(() => {});
  }, [selectedPortfolioId, sourceMode]);
  ...
  const handleExcelUpload = useCallback(async (file: File) => {
    ...
    setExcelUploadError('');
    try {
      const res = await analyticsApi.uploadExcelForBundleAnalysis(file, file.name.replace(/\.[^.]+$/, ''));
      if (!res.success || !res.data) {
      ...
      setProjectId(res.data.project_id);
      setProjectName(res.data.project_name);
      setExcelPatentCount(res.data.total_patents);
      setExcelUploadStatus(res.data.processing_status === 'completed' ? 'ready' : 'processing');
    } catch (e: any) {
      setExcelUploadStatus('error');
      setExcelUploadError(e?.message || 'Upload failed');
    }
  }, []);
  ...
  const scoreAllWithAI = async () => {
    ...
    setAiScoring(true);
    await analyticsApi.extractBundleAttributes(projectId);
    setAiScoring(false);
  ...
  const scoreSingleWithAI = async (recId: string) => {
    if (!projectId) return;
    await analyticsApi.extractBundleAttributes(projectId, [recId]);
    await loadAttributes(0);
  };
  ...
  // ── Derived stats ─────────────────────────────────────────────────────────
  const totalPatents = result?.total_patents ?? (portfolios.find(p => p.id === selectedPortfolioId)?.total_patents ?? portfolios.find(p => p.id === selectedPortfolioId)?.patent_count ?? 0);
        ...
        <div className="bg-white border-b border-neutral-200 px-6 py-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              {/* Source mode tabs */}
              <div className="flex border border-neutral-200 rounded-lg overflow-hidden bg-neutral-50">
                <button
                  onClick={() => { setSourceMode('portfolio'); setExcelUploadStatus('idle'); setExcelFile(null); }}
                  className={`px-3 py-1.5 text-sm font-medium transition-colors ${sourceMode === 'portfolio' ? 'bg-white text-neutral-900 shadow-sm' : 'text-neutral-500 hover:text-neutral-700'}`}
                >
                  Portfolio
                </button>
                <button
                  onClick={() => { setSourceMode('excel'); setSelectedPortfolioId(''); }}
                  className={`px-3 py-1.5 text-sm font-medium transition-colors ${sourceMode === 'excel' ? 'bg-white text-neutral-900 shadow-sm' : 'text-neutral-500 hover:text-neutral-700'}`}
                >
                  Excel Upload
                </button>
              </div>
              ...
              {sourceMode === 'portfolio' ? (
                    ...
                    {portfolios.map(p => (
                      <SelectItem key={p.id} value={p.id}>
                        ...
                        <span className="ml-2 text-neutral-400 text-xs">
                          {p.total_patents ?? p.patent_count ?? 0} patents
                        </span>
                      </SelectItem>
                    ))}
                  </SelectContent>
                ...
                <div className="flex items-center gap-2">
                  ...
                  {excelUploadStatus === 'processing' && (
                    <div className="flex items-center gap-2 text-sm text-neutral-600">
                      <Loader2 className="w-4 h-4 animate-spin" />
                      Processing patents…
                    </div>
                  )}
                  {excelUploadStatus === 'ready' && (
                    <div className="flex items-center gap-2">
                      <Badge variant="secondary" className="gap-1 text-xs">
                        <Check className="w-3 h-3" />
                        {excelPatentCount} patents loaded
                      </Badge>
                      <span className="text-xs text-neutral-400 max-w-[180px] truncate">{excelFile?.name}</span>
                      <Button variant="ghost" size="sm" onClick={() => { setExcelUploadStatus('idle'); setExcelFile(null); setProjectId(''); }} className="h-6 w-6 p-0">
                        <X className="w-3 h-3" />
                      </Button>
                    </div>
                  )}
                  {excelUploadStatus === 'error' && (
                    <div className="flex items-center gap-2 text-sm text-red-600">
                      ...
                      <span className="max-w-[200px] truncate">{excelUploadError}</span>
                      <Button variant="ghost" size="sm" onClick={() => { setExcelUploadStatus('idle'); setExcelFile(null); }} className="h-6 w-6 p-0">
        ...
        {!projectId ? (
          <div className="flex flex-col items-center justify-center flex-1 gap-4 py-24">
            <div className="w-16 h-16 rounded-full bg-neutral-100 flex items-center justify-center">
              {sourceMode === 'excel' ? <UploadCloud className="w-8 h-8 text-neutral-400" /> : <Package2 className="w-8 h-8 text-neutral-400" />}
            </div>
            <div className="text-center">
              {sourceMode === 'excel' ? (
                <>
                  <p className="text-neutral-700 font-medium">Upload an Excel or CSV file to get started</p>
                  <p className="text-neutral-400 text-sm mt-1">Supported formats: .xlsx, .xls, .csv with patent numbers</p>
                ...
                <>
                  <p className="text-neutral-700 font-medium">Select a portfolio to get started</p>
                  <p className="text-neutral-400 text-sm mt-1">Score patents, run bundle analysis, and build sale packages</p>
                </>
              )}
            </div>
          </div>
          ...
          <div className="flex-1 px-6 py-4 flex flex-col gap-4">
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab} className="flex-1">
              <TabsList className="bg-white border border-neutral-200 h-10">
                      ...
                      <Card key={g} className="border border-neutral-200">
                        <CardContent className="px-3 py-2 text-center">
                          ...
                          <div className="text-sm font-semibold text-neutral-800">
                            {g === 'H' || g === 'I' ? `${pctComplete.toFixed(0)}%` : '—'}
                          </div>
                          <Progress value={g === 'H' || g === 'I' ? pctComplete : 0} className="h-1 mt-1" />
                        </CardContent>
                      </Card>
                    ))}
                  </div>
